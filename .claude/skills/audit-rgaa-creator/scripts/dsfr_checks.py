...
COMPONENTS: dict[str, tuple[str, str]] = {
    ...
    "message_group": (".fr-messages-group", "components/forms-services/forms/fields.md"),
...
def load_rules() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"Catalogue de règles DSFR illisible : {RULES_PATH}: {exc}") from exc
...
def difference(
    ...
    return {
        ...
        "assessed_against": "VERSION_CIBLE" if target_version in observed_versions else ("VERSION_INDEPENDANTE" if kind == "integration" else ("MIGRATION_VERS_CIBLE" if observed_versions else "REFERENCE_VERSION_INDISPONIBLE")),
        "evidence": evidence,
...
async def inspect_page(browser: Any, page_info: dict[str, Any], config: dict[str, Any], root: Path, catalog: dict[str, Any]) -> dict[str, Any]:
    pid, url = page_info["id"], page_info["url"]
    attempts_root = root / "dsfr/preuves" / pid
    existing = [int(path.name.split("-")[-1]) for path in attempts_root.glob("attempt-[0-9][0-9][0-9]") if path.is_dir()]
    ...
    context = await browser.new_context(viewport={"width": 1440, "height": 1000}, locale="fr-FR")
    ...
    raw = await page.evaluate(r"""payload => {
    ...
    mobile = await browser.new_context(viewport={"width": 375, "height": 812}, locale="fr-FR")
    ...
    mobile_metrics = await mobile_page.evaluate("() => ({viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth})")
    await mobile.close()
    await context.close()
    ...
    for resource in raw["resources"]:
        match = re.search(r"(?:@gouvfr/dsfr@|dsfr[-./])v?(\d+\.\d+(?:\.\d+)?)", resource, re.I)
        if match:
    ...
    def add(
        ...
        differences.append(difference(pid, len(differences) + 1, rule_id, kind, component, severity, title, expected, observed, source, selector, observed_html, expected_html, recommendation, verification, failed_conditions or [], evidence_files, observed_versions, target_version, instance, observed_html_origin))

    state_candidates = [
        root / "preuves-p01-complet/P01-ETATS-DSFR-COMPLEMENTAIRES.json",
        root / f"preuves-{pid.lower()}-complet/{pid}-COLLECTE-COMPLETE.json",
        root / f"preuves-{pid.lower()}-complet/{pid}-ETATS-FORMULAIRE-COMPLEMENTAIRES.json",
    ]
    for extended_state_path in state_candidates:
        ...
        component_focus = [
            {"label": item.get("label", "Modale"), "restored": item.get("focus_restored"), "raw": item}
            for item in extended_states.get("component_states", [])
            if item.get("label") in {"Consentement", "Thème"}
        ]
        positive_labels = {item["label"] for item in component_focus if item.get("restored") is True}
        focus_checks = component_focus + [
            {"label": item.get("trigger", "Modale"), "restored": item.get("escape_focus_restored"), "raw": item}
            for item in extended_states.get("focus_traps", [])
            if item.get("trigger", "Modale") not in positive_labels
        ] + [
            {"label": item.get("label", "Modale"), "restored": item.get("focus_after_escape"), "raw": item}
            for item in extended_states.get("modal_focus_loops", [])
            if item.get("label", "Modale") not in positive_labels
        ]
        for instance, focus_check in enumerate((item for item in focus_checks if item.get("restored") is False), 1):
            label = str(focus_check["label"])
            is_theme = label in {"Paramètres d’affichage", "Thème"}
            selector = '[aria-controls="fr-theme-modal"]:not(.fr-btn--close)' if is_theme else '[aria-controls="fr-consent-modal"]:not(.fr-btn--close)'
            target_label = "Paramètres d’affichage" if is_theme else "Personnaliser"
            add("DSFR-MODAL-FOCUS-RESTORE-002", "integration", "modal", "Bloquant", f"Focus non restauré après fermeture de la modale {label}", f"Après Échap, le focus revient au déclencheur {target_label}", json.dumps(focus_check["raw"], ensure_ascii=False), selector=selector, observed_html=f'<button aria-controls="{("fr-theme-modal" if is_theme else "fr-consent-modal")}">{target_label}</button>', expected_html=f'<button aria-controls="{("fr-theme-modal" if is_theme else "fr-consent-modal")}">{target_label}</button>', recommendation="Restaurer le focus sur le déclencheur exact après la fermeture au clavier.", verification="Ouvrir la modale, parcourir ses contrôles, presser Échap puis contrôler document.activeElement.", failed_conditions=["Le focus ne revient pas sur le déclencheur."], instance=instance, observed_html_origin="MEASUREMENTS")
        ...
        for instance, field in enumerate((item for item in error_state.get("errors", []) if item.get("tag") in {"input", "textarea"} and "fr-input" in str(item.get("class", "")).split() and "fr-input--error" not in str(item.get("class", "")).split()), 1):
            selector = f"#{field.get('id')}" if field.get("id") else ".fr-input[aria-invalid=true]"
            add("DSFR-INPUT-ERROR-STATE-002", "migration" if observed_versions and target_version not in observed_versions else "integration", "input", "Majeur", "Champ en erreur sans modificateur DSFR", "Le groupe et le champ portent leurs modificateurs error et le message est relié", "fr-input--error absent", "dsfr-components/references/components/forms-services/forms/fields.md", selector=selector, observed_html=str(field.get("parentHtml") or field.get("html") or ""), expected_html='<div class="fr-input-group fr-input-group--error"><input class="fr-input fr-input--error" aria-invalid="true" aria-describedby="champ-erreur"><p id="champ-erreur" class="fr-error-text">Erreur</p></div>', recommendation="Ajouter fr-input--error au champ tout en conservant le groupe et le message relié.", verification="Déclencher la validation serveur puis inspecter chaque champ en erreur.", failed_conditions=["Le champ en erreur ne porte pas fr-input--error."], instance=instance, observed_html_origin="RENDERED_DOM")
            state_component_rules.setdefault("input", set()).add("DSFR-INPUT-ERROR-STATE-002")
            state_evidence = str(extended_state_path.relative_to(root))
            if state_evidence not in differences[-1]["evidence"]:
                differences[-1]["evidence"].append(state_evidence)

    if raw["shell"]["header"] == 0 or raw["shell"]["main"] != 1 or raw["shell"]["footer"] == 0:
        add("DSFR-SHELL-STRUCTURE-001", "integration", "page-shell", "Bloquant", "Enveloppe de page incomplète", "Un header, un main unique et un footer", json.dumps(raw["shell"], ensure_ascii=False), selector="html", observed_html="\n".join(item["html"] for item in raw["shell"]["examples"]), expected_html="<header>…</header><main id=\"content\">…</main><footer>…</footer>", recommendation="Restaurer les trois régions principales et une seule zone main.", verification="Contrôler les landmarks dans l’arbre d’accessibilité.", failed_conditions=["Nombre attendu de régions principales non observé."])
    valid_content_skips = [item for item in raw["skips"] if item["target"] and re.search(r"contenu", item["text"], re.I)]
    if not valid_content_skips:
        invalid = next((item for item in raw["skips"] if re.search(r"contenu", item["text"], re.I)), None)
        add("DSFR-SKIPLINK-CONTENT-001", "integration", "skiplinks", "Bloquant", "Lien d’évitement vers le contenu absent ou invalide", "Un lien d’évitement vers une cible de contenu existante", json.dumps(raw["skips"], ensure_ascii=False), selector=(invalid or {}).get("selector", ".fr-skiplinks"), observed_html=(invalid or {}).get("html", "<!-- Aucun lien d’évitement vers le contenu détecté -->"), expected_html="<a class=\"fr-link\" href=\"#content\">Contenu</a><main id=\"content\">…</main>", recommendation="Créer une cible de contenu existante et y transférer le focus lors de l’activation.", verification="Activer le premier lien au clavier et vérifier sa destination.", failed_conditions=["Aucune cible de contenu valide n’a été trouvée."])
    if not raw["lang"].lower().startswith("fr"):
        add("DSFR-DOCUMENT-LANG-001", "integration", "language", "Bloquant", "Langue principale française absente", "html lang=fr", f"lang={raw['lang']!r}", selector="html", observed_html=f"<html lang=\"{raw['lang']}\">", expected_html="<html lang=\"fr\">", recommendation="Déclarer la langue principale française.", verification="Inspecter l’attribut lang de html.", failed_conditions=["lang ne commence pas par fr."])
    if "width=device-width" not in raw["viewport"]:
        add("DSFR-VIEWPORT-001", "integration", "viewport", "Majeur", "Viewport mobile incomplet", "meta viewport avec width=device-width", raw["viewport"] or "absent", selector="meta[name=viewport]", observed_html=f"<meta name=\"viewport\" content=\"{raw['viewport']}\">" if raw["viewport"] else "<!-- meta viewport absente -->", expected_html="<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">", recommendation="Déclarer un viewport adapté aux terminaux mobiles.", verification="Recharger à 375 px et vérifier la largeur de mise en page.", failed_conditions=["width=device-width est absent."])
    if observed_versions and target_version not in observed_versions:
        add("DSFR-VERSION-MIGRATION-001", "migration", "version", "Majeur", "Migration DSFR à qualifier", f"Cible locale DSFR {target_version}", ", ".join(observed_versions), TARGET_DESIGN_SOURCE, selector="link[href],script[src]", observed_html="\n".join(raw["resources"]), expected_html=f"<!-- Ressources DSFR {target_version} après plan de migration -->", recommendation=f"Auditer d’abord l’intégration contre une source épinglée {observed_versions[0]}, puis planifier séparément la migration vers {target_version}.", verification="Vérifier les versions CSS et JS réellement chargées et exécuter les tests de migration.", failed_conditions=[f"La version observée diffère de la cible locale {target_version}."], observed_html_origin="RESOURCE_URLS")
    if len(observed_versions) > 1:
        add("DSFR-VERSION-COHERENCE-001", "integration", "version", "Majeur", "Versions DSFR multiples détectées", "Une version CSS/JS cohérente", ", ".join(observed_versions), TARGET_DESIGN_SOURCE, selector="link[href],script[src]", observed_html="\n".join(raw["resources"]), expected_html="<!-- Même version DSFR pour les ressources CSS et JavaScript -->", recommendation="Aligner toutes les ressources DSFR sur une seule version.", verification="Inspecter les URL et les métadonnées de version.", failed_conditions=["Plusieurs versions ont été détectées."], observed_html_origin="RESOURCE_URLS")
    if not raw["dsfrInitialized"]:
        add("DSFR-JS-INITIALIZATION-001", "integration", "javascript", "Majeur", "Initialisation JavaScript DSFR non observée", "Initialisation DSFR visible après chargement", "data-fr-js/fr-js absent", selector="html", observed_html="<html>", expected_html="<html data-fr-js=\"true\">", recommendation="Charger et initialiser le JavaScript DSFR avant les interactions.", verification="Contrôler data-fr-js après chargement.", failed_conditions=["Aucun marqueur d’initialisation DSFR n’a été observé."])
    if raw["duplicateIds"]:
        add("DSFR-HTML-ID-UNIQUE-001", "integration", "html", "Bloquant", "Identifiants dupliqués dans les composants", "Identifiants uniques", ", ".join(raw["duplicateIds"]), selector=", ".join(f"#{value}" for value in raw["duplicateIds"]), observed_html="\n\n".join(instance["html"] for group in raw["duplicateInstances"] for instance in group["instances"]), expected_html="<!-- Un identifiant unique par élément et par variante de gabarit -->", recommendation="Rendre uniques les identifiants des variantes desktop et mobile puis mettre à jour leurs références.", verification="Recalculer les occurrences de chaque id dans le DOM rendu.", failed_conditions=[f"Identifiants présents plusieurs fois : {', '.join(raw['duplicateIds'])}."])
    if raw["grid"]["containers"] == 0 or raw["grid"]["rows"] == 0:
        add("DSFR-GRID-STRUCTURE-001", "migration" if observed_versions and target_version not in observed_versions else "integration", "grid", "Majeur", "Grille DSFR non observée", "fr-container et fr-grid-row pour une page complète", json.dumps(raw["grid"], ensure_ascii=False), selector="body", observed_html=raw["bodyOpenTag"], expected_html="<div class=\"fr-container\"><div class=\"fr-grid-row\">…</div></div>", recommendation="Utiliser les primitives de grille lorsque le gabarit de page le requiert.", verification="Inspecter les conteneurs, lignes et colonnes à chaque breakpoint.", failed_conditions=["fr-container ou fr-grid-row est absent."])
    if "Marianne" not in raw["fontFamily"]:
        add("DSFR-TYPOGRAPHY-MARIANNE-001", "migration" if observed_versions and target_version not in observed_versions else "integration", "typography", "Majeur", "Police Marianne non observée sur le corps", "Marianne avec repli Arial", raw["fontFamily"], selector="body", observed_html=raw["bodyOpenTag"], expected_html="body { font-family: Marianne, arial, sans-serif; }", recommendation="Vérifier le chargement des fontes et les tokens typographiques DSFR.", verification="Contrôler la police calculée du corps et des composants.", failed_conditions=["La police calculée ne contient pas Marianne."], observed_html_origin="COMPUTED_VALUE")
    if raw["mobile"]["scrollWidth"] > raw["mobile"]["viewport"]:
        add("DSFR-RESPONSIVE-OVERFLOW-001", "integration", "responsive", "Majeur", "Débordement horizontal mobile", "Aucun débordement à 375 px", json.dumps(raw["mobile"]), selector="html", observed_html=f"<!-- scrollWidth={raw['mobile']['scrollWidth']}, viewport={raw['mobile']['viewport']} -->", expected_html="<!-- scrollWidth <= viewport -->", recommendation="Identifier l’instance débordante et corriger ses contraintes de largeur.", verification="Tester à 320 px et 375 px avec zoom et espacement du texte.", failed_conditions=["scrollWidth dépasse la largeur du viewport."], observed_html_origin="MEASUREMENTS")
    missing_legal = [name for name in ("accessibility", "legal", "personalData", "cookies") if not raw["legal"][name]]
    if missing_legal:
        add("DSFR-FOOTER-LEGAL-001", "integration", "footer", "Majeur", "Liens ou mentions de pied de page incomplets", "Accessibilité, mentions légales, données personnelles et cookies", ", ".join(missing_legal), "dsfr-components/references/components/structure/footer.md", selector=raw["legal"]["selector"], observed_html=raw["legal"]["html"], expected_html="<footer class=\"fr-footer\">… liens institutionnels et politiques …</footer>", recommendation="Ajouter les liens institutionnels manquants dans la zone documentée du pied de page.", verification="Contrôler les intitulés, destinations et disponibilité des liens.", failed_conditions=[f"Mentions absentes : {', '.join(missing_legal)}."])
    ...
    for item in raw["ruleResults"]:
        ...
        kind = "integration" if rule.get("version_scope") == "VERSION_INDEPENDENT" else ("migration" if observed_versions and target_version not in observed_versions else "integration")
        add(rule["rule_id"], kind, rule["component"], rule["severity"], rule["title"], rule["expected"], "; ".join(item["failed_conditions"]), rule["source"], selector=item["selector"], observed_html=item["observed_html"], expected_html=rule["expected_html"], recommendation=rule["recommendation"], verification=rule["verification"], failed_conditions=item["failed_conditions"], instance=item["instance"])
    ...
    for name, item in sorted(raw["components"].items()):
        ...
        elif state_rules or any(result["signal_status"] == "FAIL_CANDIDATE" for result in results):
            status = "ECART_OBSERVE"
        else:
        ...
        inventory.append({
            ...
            "status": status,
            "rules_executed": sorted({result["rule_id"] for result in results} | state_rules),
            "source": "dsfr-components/references/" + COMPONENTS[name][1],
            "samples": item["samples"],
        })

    comparison_mode = "VERSION_CIBLE" if target_version in observed_versions else ("MIGRATION_VERS_CIBLE" if observed_versions else "REFERENCE_VERSION_INDISPONIBLE")
    result = {
        "schema_version": 2,
        "page": page_info,
        "audited_at": dt.datetime.now(dt.timezone.utc).isoformat(timespec="seconds"),
        "version": {"observed": observed_versions, "target": target_version, "comparison_mode": comparison_mode, "exact_observed_reference_available": target_version in observed_versions},
        "reference_version": target_version,
        "detected_versions": observed_versions,
        "status": "SIGNAUX D’ÉCART OBSERVÉS" if differences else "AUCUN ÉCART OBSERVÉ SUR LES RÈGLES EXÉCUTÉES",
        "claim": "Aucune conformité DSFR globale n’est revendiquée.",
        "inventory": inventory,
        "rule_results": raw["ruleResults"],
        "global_checks": raw,
        "differences": differences,
        "sources": [VERIFY_SOURCE, TARGET_DESIGN_SOURCE, "audit-dsfr-complet/rules/dsfr-rules.json", "dsfr-components/assets/dsfr_complete_library.json"],
        "not_verified": ["droit d’usage de la marque de l’État", "exhaustivité officielle des composants", "fidélité visuelle exhaustive", "lecteur d’écran réel"] + ([] if target_version in observed_versions else [f"intégration exacte contre une source locale DSFR {observed_versions[0] if observed_versions else 'observée'}"]),
        "evidence": {"attempt": attempt, "raw": evidence_path, "desktop_screenshot": desktop_path, "mobile_screenshot": mobile_path},
    }
...
async def main(config_path: Path) -> int:
    ...
    async with async_playwright() as playwright:
        ...
        for page_info in config["sample"]:
            try:
                result = await inspect_page(browser, page_info, config, root, catalog)
        ...
        await browser.close()
    dump(root / "logs/dsfr-checks-summary.json", {"failures": failures, "pages": len(config["sample"]), "rules": len(catalog["rules"]), "target_version": catalog["target_version"]})
