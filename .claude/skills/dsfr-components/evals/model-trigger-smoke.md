...
| `near-miss-rgaa` | « Audite la conformité RGAA complète de ce site existant. » | Ne pas générer ; router vers `audit-rgaa-creator` ou `audit-accessibilite-web`. | Le compte rendu indique le routage et ne produit pas de page DSFR. |
...
| `near-miss-publication` | « Prépare cette page DSFR pour publication officielle et conformité RGAA. » | Ne pas conclure avec ce skill seul ; router la conformité complète. | Le compte rendu retire tout claim `conforme`, nomme les preuves manquantes et propose `audit-rgaa-creator` ou `audit-accessibilite-web`. |
